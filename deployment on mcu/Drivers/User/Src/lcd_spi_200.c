...
void _LCD_DrawRect(uint16_t x, uint16_t y, uint16_t rect_w, uint16_t rect_h, uint16_t img_w, uint16_t * _LCD_Buff)
{
    uint32_t i, j;

    for (j = y; j < y + rect_h; j++)
    {
        for (i = x; i < x + rect_w; i++)
        {
            // 判断是否是边界点（绘制边框）
            if (i == x || i == x + rect_w - 1 || j == y || j == y + rect_h - 1)
            {
                _LCD_Buff[j * img_w + i] = LCD.Color;  
            }
        }
    }
...
void RGB888_to_RGB565_image(const uint8_t *src_img, uint16_t *dest_img, int width, int height) {
    for (int i = 0; i < width * height; i++) {
        uint8_t red = src_img[i * 3];
        uint8_t green = src_img[i * 3 + 1];
        uint8_t blue = src_img[i * 3 + 2];

        // RGB888 转换为 RGB565
        uint16_t r = red >> 3;      // 红色5位
        uint16_t g = green >> 2;    // 绿色6位
        uint16_t b = blue >> 3;     // 蓝色5位

        dest_img[i] = (r << 11) | (g << 5) | b;
    }
}


// RGB565 转换为 RGB888
void RGB565_to_RGB888_image(const uint16_t *src_img, uint8_t *dest_img, int width, int height) {
    for (int i = 0; i < width * height; i++) {
        uint16_t rgb565 = src_img[i];
